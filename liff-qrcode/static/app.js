...
let QrResult = function(onCloseCallback) {
    ...
    this.__onScanSuccess = function(decodedText, decodedResult, scanType) {
        ...
        decodedText = decodedText.substring(0,10);
        stdform = /^[A-Z]{2}\d{8}$/;
        if (!stdform.test(decodedText)) {
            scanResultCodeType.innerText
            = "發票格式錯誤";
            scanResultText.innerText = "";
            invoiceTitle.innerText = "";
            ...
            scanResultParsed.appendChild(createParsedResult("", codeType));
            container.style.display = "block";
        } else {
            postData('https://linefresh-tiehua.herokuapp.com/invoice', {user_id: uid, invoice_num: decodedText}).then(data =>{
                if (data['result']) {
                    scanResultCodeType.innerText = "發票掃描成功";
                    scanResultText.innerText = decodedText;
                    let codeType = detectType(decodedText);
                    invoiceTitle.innerText = "發票號碼";
                    ...
                    scanResult.innerText = "已儲存您的發票號碼";
                    container.style.display = "block";

                }else {
                    scanResultCodeType.innerText = "此發票已掃描過";
                    scanResultText.innerText = decodedText;
                    let codeType = detectType(decodedText);
                    invoiceTitle.innerText = "發票號碼";
...
docReady(function() {
    ...
    Logger.logScanStart();
    liff.init({
        liffId: '1656598921-Xr4AEy3Y'
    }).then(() => {
        liff.getProfile().then(profile => {
            const userId = profile.userId;
            uid = userId;
            window.alert("掃描任一在鐵花村消費的發票，即可完成任務");
        });
    });
